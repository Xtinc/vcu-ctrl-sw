...
{
  public:
    UnitDropProxy(asio::io_context &ioc, uint16_t listen_port, uint16_t dst_port, uint8_t target_units_num,
                  uint8_t drop_idx)
        : socket_(ioc, asio::ip::udp::endpoint(asio::ip::udp::v4(), listen_port)),
          dst_(asio::ip::make_address("127.0.0.1"), dst_port), buf_(new uint8_t[TEST_UDP_DATAGRAM_SIZE]),
          target_units_num_(target_units_num), drop_idx_(drop_idx), dropped_(0), forwarded_(0)
    {
        do_receive();
    }
    ...
    UnitDropProxy(const UnitDropProxy &) = delete;
    UnitDropProxy &operator=(const UnitDropProxy &) = delete;
                                   ...
                                   [this](const asio::error_code &ec, size_t n) {
                                       ...
                                       if (!ec && n > 0)
                                       {
                                           ...
                                           if (n > TRX_HEADER_SIZE)
                                           {
                                               ...
                                               drop = TRXUnit::validate(&head) && head.units_num == target_units_num_ &&
...
static std::vector<uint8_t> make_recovery_test_payload(size_t size, uint8_t seq)
{
...
static bool run_one_recovery_case(uint8_t units_num, uint8_t drop_idx, size_t payload_size, uint16_t port_base)
{
    auto &ioc = BG_SERVICE;
    ...
    receiver->set_receive_callback([&](const std::vector<QueueFrame> &frames, bool) {
        ...
        for (const auto &frame : frames)
        {
            ...
            const auto expected = make_recovery_test_payload(payload_size, seq);
    ...
    UnitDropProxy proxy(ioc, proxy_port, receiver_port, units_num, drop_idx);
    ...
    {
        const auto payload = make_recovery_test_payload(payload_size, static_cast<uint8_t>(i));
    ...
    return sent && proxy.dropped() >= frame_count && proxy.forwarded() >= frame_count * (units_num - 1u) &&
           deliveries == frame_count && errors == 0;
}

static int run_dup_and_xor_tests()
{
    struct TestCase
    {
        const char *mode;
    ...
    const TestCase cases[] = {
        {"dup", static_cast<uint8_t>(MAX_DUP_PACKET_NUM_PER_GROUP + TRX_DUP_FEC_REDUNDANCY), 0, 1},
        {"dup", static_cast<uint8_t>(MAX_DUP_PACKET_NUM_PER_GROUP + TRX_DUP_FEC_REDUNDANCY), 1, 97},
        {"dup", static_cast<uint8_t>(MAX_DUP_PACKET_NUM_PER_GROUP + TRX_DUP_FEC_REDUNDANCY), 0, MAX_TRX_DATA_SIZE},
        {"dup", static_cast<uint8_t>(MAX_DUP_PACKET_NUM_PER_GROUP + TRX_DUP_FEC_REDUNDANCY), 1, MAX_TRX_DATA_SIZE},
        {"xor", static_cast<uint8_t>(MAX_XOR_PACKET_NUM_PER_GROUP + TRX_XOR_FEC_REDUNDANCY), 0, MAX_TRX_DATA_SIZE + 1},
        {"xor", static_cast<uint8_t>(MAX_XOR_PACKET_NUM_PER_GROUP + TRX_XOR_FEC_REDUNDANCY), 1,
         MAX_XOR_GROUP_DATA_SIZE},
        {"xor", static_cast<uint8_t>(MAX_XOR_PACKET_NUM_PER_GROUP + TRX_XOR_FEC_REDUNDANCY), 2,
         MAX_XOR_GROUP_DATA_SIZE},
    };

    std::cout << "\nReliableUDP DUP and XOR FEC tests\n";
    print_divider();
    ...
    {
        const bool ok = run_one_recovery_case(t.units_num, t.drop_idx, t.payload_size, port_base);
        port_base = static_cast<uint16_t>(port_base + 3);
        std::cout << "  " << (ok ? "PASS" : "FAIL") << "  " << t.mode << " drop unit "
                  << static_cast<unsigned>(t.drop_idx) << ", payload " << t.payload_size << " bytes\n";
        if (!ok)
            ++failures;
    }
    ...
    print_divider();
    if (failures == 0)
        std::cout << "  All DUP and XOR FEC tests passed.\n";
    else
        std::cout << "  " << failures << " DUP/XOR FEC test(s) FAILED.\n";
    print_divider();
    std::cout << '\n';
...
int main(int argc, char *argv[])
{
    ...
    if (run_rtt_tests() != 0 || run_dup_and_xor_tests() != 0)
    {
        return 1;
    }
