...
public:
     ...
     */
    int64_t get_offset_ns() const { return offset_ns_.load(std::memory_order_relaxed); }

    /**
     * @brief Get last measured round-trip time in nanoseconds
     * @return RTT in nanoseconds
     */
    int64_t get_rtt_ns() const { return rtt_ns_.load(std::memory_order_relaxed); }

    /**
     * @brief Check if synchronization is active
     */
    bool is_synchronized() const { return synchronized_.load(std::memory_order_relaxed); }
