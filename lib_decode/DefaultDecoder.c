...
static void AL_sDecoder_CallEndParsing(AL_TDecCtx* pCtx, AL_TBuffer* pParsedFrame, int iParsingID)
{
  ...
  {
    (void)numHandles;
    DBG_ASSERT_COND(numHandles == 1);
  }
  DBG_ASSERT_COND(iParsingID < numHandles);
...
static bool AL_sDecoder_TryDisplayOneFrame(AL_TDecCtx* pCtx, int iFrameID)
{
  ...
  (void)pPtrIsNotNull;
  DBG_ASSERT_COND(pPtrIsNotNull != NULL);
...
void AL_Default_Decoder_EndDecoding(void* pUserParam, AL_TDecPicStatus const* pStatus)
{
  ...
  {
    VIDEO_ERROR_PRINT("Invalid command detected in the bitstream. Ending decoding.");
    DBG_ASSERT_COND(0);
    return;
  }
  ...
  {
    VIDEO_ERROR_PRINT("Decoder timeout detected. Resetting the decoder.");
  }
...
{
  (void)eCodec;
  DBG_ASSERT_COND(checkSeiUUID(pBufs, pNal, eCodec, iTotalSize));
  int iStart = 7;
  ...
  for(int iNal = 0; iNal < iNalCount; ++iNal)
  {
    ...
    if(!isVcl(eCodec, eNUT))
    {
      ...
      if(isPrefixSei(eCodec, eNUT) && checkSeiUUID(pBuf, pNal, eCodec, AL_Buffer_GetSize(pStream)))
      {
        ...
        DBG_ASSERT_COND(pCtx->iNumSlicesRemaining > 0);
      }
...
/*****************************************************************************/
bool AL_DecodeOneNal(AL_TAup* pAUP, AL_TDecCtx* pCtx, AL_ENut nut, uint32_t uNutSize, bool bIsLastAUNal, int* iNumSlice)
{
  (void)uNutSize;
...
static bool RefillStartCodes(AL_TDecCtx* pCtx, AL_TBuffer* pStream)
{
  ...
  do
  {
    ...
    {
      VIDEO_INFO_PRINT("Start code queue was full, degraded mode, retrying...");
      Rtos_Sleep(1);
    }
  }
...
static bool GetNextNal(AL_TDecCtx* pCtx, AL_TNal* nals, int iNalCount, int iLastVclNalInAU, int* iNal, AL_DecodeNalStep* step)
{
  ...
  DBG_ASSERT_COND(isITU(pCtx->pChanParam->eCodec) && "Unsupported codec");
  ...
  while(true)
  {
    ...
    case SEND_LAST_VCL:
    {
      DBG_ASSERT_COND(*iNal == iLastVclNalInAU);
...
bool AL_Default_Decoder_PreallocateBuffers(AL_TDecoder* pAbsDec)
{
  AL_TDecoder* pDec = (AL_TDecoder*)pAbsDec;
  AL_TDecCtx* pCtx = &pDec->ctx;
  DBG_ASSERT_COND(!pCtx->bAreBuffersAllocated);
...
{
  DBG_ASSERT_COND(isAVC(pSettings->eCodec));
...
#define SAFE_ALLOC(pCtx, pMD, uSize, name) \
  ...
  DBG_ASSERT_COND(pCtx->eosBuffer);
