...
public:
  ...
  using EncodedFrameCallback =
    std::function<void(const uint8_t* pData, size_t size, bool isKeyFrame)>;
  ...
  /* ---- 只读访问（供上层调试/数据源适配使用） ---- */
  TFourCC       getSrcFourCC()   const { return m_srcFourCC;      }
  int           getPitchY()      const { return m_pitchY;         }
  uint8_t       getSrcBitDepth() const { return m_cfg.uBitDepth;  }
  AL_EChromaMode getChromaMode() const { return m_cfg.eChromaMode; }

private:
  /* ---- SDK 回调（静态转发到实例方法） ---- */
  static void sdkCallback(void* pUserParam,
                          AL_TBuffer* pStream,
  ...
  /* ---- 帧数据拷贝辅助 ---- */
  void copyFrameData(AL_TBuffer* pDstBuf,
