...
static AL_TFrameInfo* GetNextFrameInfo(AL_TFrameInfoPool* pFrameInfoPool)
{
  ...
  {
    DBG_ASSERT_COND(bNextPoolIDAvailable);
    return NULL;
  }
...
{
  DBG_ASSERT_COND(iID != INVALID_POOL_ID);
...
static void EndEncoding(void* pUserParam, AL_TEncPicStatus* pPicStatus, AL_64U streamUserPtr)
{
  ...
  DBG_ASSERT_COND(streamId >= 0 && streamId < AL_MAX_STREAM_BUFFER);
  DBG_ASSERT_COND(pCtx->tLayerCtx[iLayerID].iCurStreamRecv == streamId);
...
{
  DBG_ASSERT_COND(pSettings->NumLayer > 0);
