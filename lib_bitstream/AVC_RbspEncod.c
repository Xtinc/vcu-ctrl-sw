...
{
    DBG_ASSERT_COND(pHrd->cpb_cnt_minus1[0] < MAX_NUM_CPB);
...
static void writeSpsData(AL_TBitStreamLite *pBS, AL_TAvcSps const *pSps)
{
    ...
    if ((pSps->profile_idc == AVC_PROFILE_IDC_HIGH) || (pSps->profile_idc == AVC_PROFILE_IDC_HIGH10) ||
        (pSps->profile_idc == AVC_PROFILE_IDC_HIGH_422) || (pSps->profile_idc == AVC_PROFILE_IDC_HIGH_444_PRED) ||
        (pSps->profile_idc == AVC_PROFILE_IDC_CAVLC_444_INTRA) || (pSps->profile_idc == 83) ||
        (pSps->profile_idc == 86) || (pSps->profile_idc == 118) || (pSps->profile_idc == 128))
    {
    ...
    DBG_ASSERT_COND(pSps->pic_order_cnt_type != 1);
    ...
    if (pSps->vui_parameters_present_flag)
    {
        ...
        DBG_ASSERT_COND(pSps->vui_param.bitstream_restriction_flag == 0);
    }
}
...
static void writePps(AL_TBitStreamLite *pBS, AL_TPps const *pIPps)
{
    ...
    DBG_ASSERT_COND(pPps->num_slice_groups_minus1 == 0);
    ...
    DBG_ASSERT_COND(pPps->chroma_qp_index_offset >= -12 && pPps->chroma_qp_index_offset <= 12);
    ...
    if (pPps->transform_8x8_mode_flag || (pPps->second_chroma_qp_index_offset != pPps->chroma_qp_index_offset))
    {
        AL_BitStreamLite_PutU(pBS, 1, pPps->transform_8x8_mode_flag);
        AL_BitStreamLite_PutU(pBS, 1, pPps->pic_scaling_matrix_present_flag);
        DBG_ASSERT_COND(pPps->pSPS != NULL);
        writeScalingMatrix(pBS, pPps->pic_scaling_matrix_present_flag, pPps->pSPS);
        DBG_ASSERT_COND(pPps->second_chroma_qp_index_offset >= -12 && pPps->second_chroma_qp_index_offset <= 12);
...
/******************************************************************************/
static void writeSeiBufferingPeriod(AL_TBitStreamLite *pBS, AL_TSps const *pISps, int iCpbInitialDelay,
                                    int iCpbInitialOffset)
{
...
/******************************************************************************/
static void writeSeiPictureTiming(AL_TBitStreamLite *pBS, AL_TSps const *pISps, int iCpbRemovalDelay,
                                  int iDpbOutputDelay, int iPicStruct)
{
    AL_TAvcSps *pSps = (AL_TAvcSps *)pISps;
    // Table D-1
    static const int PicStructToNumClockTS[9] = {1, 1, 1, 2, 2, 3, 3, 2, 3};
    ...
    if (pSps->vui_param.pic_struct_present_flag)
    {
        ...
        DBG_ASSERT_COND(iPicStruct <= 8 && iPicStruct >= 0);
...
static inline bool isForce4BytesCode(AL_EStartCodeBytesAlignedMode eMode, int nut)
{
    switch (eMode)
    {
    case AL_START_CODE_AUTO:
        return nut >= AL_AVC_NUT_PREFIX_SEI && nut < AL_AVC_NUT_UNSPEC_24;
    case AL_START_CODE_3_BYTES:
        return false;
    case AL_START_CODE_4_BYTES:
        return true;
    default:
        return nut >= AL_AVC_NUT_PREFIX_SEI && nut < AL_AVC_NUT_UNSPEC_24;
    }
